...
export default function StepSessionResult({ words, unitId, mode, wordStats, skillType = 'vocabulary' }) {
  ...
  const itemLabelLower = isGrammar ? 'cấu trúc' : 'từ';
    ...
    <div className="wf-main-content">
      ...
      <div className="wf-topbar">
        <div className="wf-step-counter">
          ...
          <div className="wf-step-counter-item">{itemLabel}: <strong>{totalItems}</strong> / {totalItems}</div>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '850px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>Bạn đã hoàn thành các {itemLabelLower} của Unit {unitId}: Daily Life. Đây là tóm tắt</p>
        </div>
        ...
        <div style={{ width: '100%', marginBottom: '16px', fontWeight: 'bold', color: 'var(--text-primary)' }}>Chi tiết từng {itemLabelLower}</div>
            ...
            <div key={idx} style={{ 
              ...
              <div className="flex-row items-center gap-12">
                ...
                <div style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--text-primary)' }}>
                  {w.word || w.title}{!isGrammar && w.ipa ? (
                    <span style={{ fontWeight: 'normal', color: 'var(--text-secondary)', marginLeft: '8px' }}>{w.ipa}</span>
                  ) : null}
                </div>
              </div>
        ...
        <div style={{ 
          ...
          <div style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>{reviewCount} {itemLabelLower} yếu sẽ được ôn tập sớm hơn</div>
        </div>
