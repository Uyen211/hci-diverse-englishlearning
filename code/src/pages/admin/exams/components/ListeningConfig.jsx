...
export default function ListeningConfig({ question, onChange, errors }) {
  const [activeSubIdx, setActiveSubIdx] = useState(0);

  const handleAudioUpload = () => {
    // Mock audio file selection
    onChange({ ...question, audioFile: 'audio_listening_sec1.mp3' });
  };
    ...
    <div className="flex flex-col gap-5 w-full">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        <div className="input-group">
          <label>Tải file Audio đề thi (.mp3) *</label>
          <div
            onClick={handleAudioUpload}
            className="upload-box"
            style={{
              padding: '16px',
              border: errors?.audioFile ? '2px dashed var(--error)' : '2px dashed rgba(78, 86, 192, 0.2)',
              backgroundColor: errors?.audioFile ? 'rgba(239, 68, 68, 0.02)' : 'rgba(78, 86, 192, 0.02)',
          ...
          >
            {question.audioFile ? (
              <>
                <span style={{ fontWeight: '600', color: 'var(--primary)' }}>[ {question.audioFile} ]</span>
                <span style={{ fontSize: '11px', opacity: 0.6 }}>Kéo thả file mới để thay thế</span>
              </>
            ) : (
              <span style={{ fontWeight: '600', color: 'var(--text-secondary)' }}>Chọn file âm thanh (.mp3)</span>
            )}
          </div>
        ...
        <div className="input-group">
          <label>Lời thoại / Script bài nghe</label>
          <textarea
            className="input-field"
            rows={3}
            value={question.script || ''}
            onChange={(e) => onChange({ ...question, script: e.target.value })}
